         ...
         private void TriggerCampaign(string cid, string cdata)
         {
             ...
             if (!string.IsNullOrEmpty(cid) && ID.TryParse(cid, out scId))
             {
                 ...
                 if (Tracker.Current.CurrentPage != null && Tracker.Current.Interaction != null)
                 {
                    ...
                    var campaignActivityDefinition = definitionManager.Get(scId.Guid, System.Globalization.CultureInfo.InvariantCulture);

                    Tracker.Current.CurrentPage.TriggerCampaign(campaignActivityDefinition);
                }
             }
         }       
    }
